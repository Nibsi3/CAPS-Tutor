/**
 * Improved parser for questions from JSON - uses question_text field which is cleaner
 * Properly extracts full questions, detects types, handles sub-questions
 */

/**
 * Extract all questions from a question_text string with proper parsing
 */
function extractQuestionsFromText(questionText, fullText, questionType, hasImages, questionNumber) {
  const questions = [];
  
  // Prefer question_text over full_text as it's cleaner
...
export function parseQuestionsToStructure(jsonData) {
  ...
  (jsonData.questions || []).forEach((q, idx) => {
    // Use question_text if available (cleaner), otherwise fall back to full_text
    ...
    if (hasImages) {
      imageMap.set(questionNumber?.toString() || idx.toString(), q.images);
    }
